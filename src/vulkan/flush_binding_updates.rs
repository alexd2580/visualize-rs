...
use crate::{
    ...
    utils::map_snd,
    vulkan::{
        resources::{buffer::Buffer, image_view::ImageView, sampler::Sampler},
        Vulkan,
    },
};

use super::resources::shader_module::analysis::DescriptorInfo;
...
impl Vulkan {
    pub unsafe fn flush_binding_updates(&mut self) -> Result<(), Error> {
        ...
        for resources in &self.shader_resources {
            for (name, image_infos) in &image_descriptor_data {
                if let Some(declaration) = resources.shader_module.variable_declaration(name) {
                    let storage = declaration.storage();
                    let set_index = declaration.set_index();
                    let sets = &resources.descriptor_sets[set_index];
            ...
            for (name, buffer_infos) in &buffer_descriptor_data {
                if let Some(declaration) = resources.shader_module.block_declaration(name) {
                    let storage = declaration.storage();
                    let set_index = declaration.set_index();
                    let sets = &resources.descriptor_sets[set_index];
                    let binding = declaration.binding()?;
